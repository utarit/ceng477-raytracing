...
{
	Vector3f result;
	float distance = pow((p.x - position.x), 2) + pow((p.y - position.y), 2) + pow((p.z - position.z), 2);

	result.r = intensity.r / distance;
	result.g = intensity.g / distance;
	result.b = intensity.b / distance;
	return result;
}
